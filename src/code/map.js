...
function createMap(nld, data, year, currentProvince, topic, titel, subtitel, pyramidSettings, barSettings){
   ...
   var svg = d3.select("#container1")
    ...
    svg.selectAll("path")
        ...
        .on("click", function(d){
            // Previous clicked map color reset
            svg.select("#" + currentProvince).style("fill", function(d) {
            ...
            // Update pyramid and barchart
            currentProvince = province;
            provinceDataOne = getProvinceData(data, "Limburg");
            provinceDataTwo = getProvinceData(data, province);
            updatePyramid(provinceDataOne, provinceDataTwo, province, pyramidSettings);
            updateBarchart(provinceDataOne, provinceDataTwo, province, topic, titel, subtitel, barSettings);
        });
...
function updateMap(nld, data, year, currentProvince, topic, titel, subtitel, settings){
    ...
    var mapsubtitle = settings.mapsubtitle;
    var maptitle = settings.maptitle
